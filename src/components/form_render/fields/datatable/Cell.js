...
const TextCell = ({data, rowIndex, colIndex, actionRule, element, lastRow, role}) => {
  ...
  return (
    <TextInput
      ...
      value={data}
      editable={role.edit && (userRole.edit || userRole.submit)}
      onChangeText={e => {
...
const NumberCell = ({data, rowIndex, colIndex, actionRule, element, lastRow, role}) => {
  const {colors, fonts} = useTheme();
  const {tableData, setTableData, cellWidth} = useContext(DataTableContext);
  const preview = formStore(state => state.preview);
  const userRole = formStore(state => state.userRole);
...
const DateCell = ({data, rowIndex, colIndex, actionRule, element, lastRow, role}) => {
  const {colors, fonts} = useTheme();
  const {tableData, setTableData, cellWidth} = useContext(DataTableContext);
  const userRole = formStore(state => state.userRole);
  const preview = formStore(state => state.preview);
  ...
  return (
    <View key={rowIndex}>
      <TouchableOpacity onPress={() => setVisible(true)} disabled={!(role.edit && (userRole.edit || userRole.submit))}>
...
const TimeCell = ({data, rowIndex, colIndex, actionRule, element, lastRow, role}) => {
  const {colors, fonts} = useTheme();
  const {tableData, setTableData, cellWidth} = useContext(DataTableContext);
  const userRole = formStore(state => state.userRole);
  const preview = formStore(state => state.preview);
...
const CheckBoxCell = ({data, rowIndex, colIndex, actionRule, element, lastRow, role}) => {
  const {colors, fonts} = useTheme();
  const {tableData, setTableData, cellWidth} = useContext(DataTableContext);
  const userRole = formStore(state => state.userRole);
  const preview = formStore(state => state.preview);
...
const MenuCell = ({data, rowIndex, colIndex, header, actionRule, element, lastRow, role}) => {
