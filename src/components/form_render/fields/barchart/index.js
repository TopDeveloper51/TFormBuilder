...
const BarChartSubField = ({element, role}) => {
...
const BarChartField = ({element, index, role}) => {
  const updateFormData = formStore(state => state.updateFormData);
  return useMemo(() => <BarChartSubField  onClickUpdateField={updateFormData} element={element} index={index} role={role}/>, [element, index, role]);
};
