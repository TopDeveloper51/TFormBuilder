...
const Button = props => {
  const {element, onProcess, role} = props;
  ...
  const onClick = () => {
    ...
    if (element.meta.function === 'Dialog') {
      setVisibleDialogFieldDlg({...visibleDialogFieldDlg, visible: true, data: element.meta.dialogData || [], role: role});
    }
  };
