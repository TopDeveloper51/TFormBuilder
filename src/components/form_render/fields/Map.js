...
const MapChildComponent = ({element, index, onClickUpdateField, role}) => {
            ...
            if (e.type === 'circleFence') {
              return (
                <List.Item
                  ...
                  right={props => {
                    if ((userRole.edit || userRole.submit) && role.edit) {
                      return (
...
const Map = ({element, index, role}) => {
  const updateFormData = formStore(state => state.updateFormData);
  const formValue = formStore(state => state.formValue);
  return useMemo(() => <MapChildComponent element={element} index={index} onClickUpdateField={updateFormData} role={role}/>, [element, index, JSON.stringify(formValue[element.field_name]), role]);
};
